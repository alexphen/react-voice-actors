...
try :
    ...
    for i in parsed_master["actors"] :
        ...
        for r in curr["roles"] : ###############################################################################
            currRole = VA.Role(curr["roles"][r]["character"], curr["roles"][r]["img"], curr["roles"][r]["charID"], curr["roles"][r]["fav"])
            currRole.shows = r["shows"]
            currRole.showIDs = r["showIDs"]
            entry.roles.append(currRole)
    ...
    try:
        with open(name, 'r', encoding='utf8') as f :
            ...
            for i in parsedList["actors"] :
                ...
                for r in curr["roles"] :
                    currRole = VA.Role(curr["roles"][r]["character"], curr["roles"][r]["img"], curr["roles"][r]["charID"], curr["roles"][r]["fav"])
                    currRole.showIDs = r["showID"]
                    currRole.titles = r["titles"]
                    entry.roles.append(currRole)
                    entry.charIDs.append(r["charID"])
            ...
            try :
                ...
                for voice in character["voice_actors"] :
                    if voice["language"] == 'Japanese' :
                        ...
                        if actorID not in myList.actors :
                            ...
                            if not failed :
                                try :
                                    ...
                                    aImg = actorJSON["data"]["images"]["jpg"]["image_url"]
                                except :
                                    print(actorJSON)
                            
                            entry = VA.Actor(VA.rename(actor), actorID, aFavs, aImg)
                            currRole = VA.Role(char, img, charID, favs)
                            currRole.showIDs.append(showID)
                            currRole.titles.append(title)
                            entry.roles[charID] = currRole

                            entry.charIDs.append(charID)
                            myList.actors[actorID] = entry
                            currShow.actorIDs.append(actorID)
                            
                            actorResponse.close()
                        else :
                            # print("Already have actor for: ", char)
                            # if actor is already in myList append char to their list
                            currActor = myList.actors[actorID]
                            # if the character is not registered yet
                            if charID not in currActor.charIDs :
                                currRole = VA.Role(char, img, charID, favs)
                                currRole.showIDs.append(showID)
                                currRole.titles.append(title)
                                currActor.roles[charID] = currRole
                                currActor.charIDs.append(charID)
                            # if the character exists in another show already
                            else :
                                currActor.roles[charID].titles.append(title)
                                currActor.roles[charID].showIDs.append(showID)
                            currShow.actorIDs.append(actorID)
